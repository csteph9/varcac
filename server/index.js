...
app.post('/api/element-definitions', async (req, res) => {
  ...
  if (!name || !nameRegex.test(name)) {
    return res.status(400).json({ error: 'Invalid element name. Use letters/numbers/underscore; cannot start with a number.' })
  }
  const scopeVal = isValidScope(scope) ? scope : 'payout'
  try { validateFormulaSyntax(formula) }
  catch (e) { return res.status(e.code === 'FORBIDDEN' ? 400 : 422).json({ error: 'Invalid formula', detail: e.message }) }

  try {
    const [ins] = await pool.execute(
      `INSERT INTO element_definition (name, scope, formula, notes)
       VALUES (?,?,?,?)`,
      [name, scopeVal, formula || null, notes || null]
    )
    res.status(201).json({ id: ins.insertId, name, scope: scopeVal })
  } catch (e) {
    if (e?.errno === 1062) return res.status(409).json({ error: 'Element name already exists' })
    res.status(500).json({ error: 'Failed to create element', detail: e.message })
  }
})
...
app.put('/api/element-definitions/:id', async (req, res) => {
  ...
  try { if (formula !== undefined) validateFormulaSyntax(formula) }
  catch (e) { return res.status(e.code === 'FORBIDDEN' ? 400 : 422).json({ error: 'Invalid formula', detail: e.message }) }

  const sets = [], params = []
  if (name !== undefined)    { sets.push('name = ?');    params.push(name) }
  if (scope !== undefined)   { sets.push('scope = ?');   params.push(scope) }
  if (formula !== undefined) { sets.push('formula = ?'); params.push(formula || null) }
...
app.get('/api/element-definitions', async (_req, res) => {
  try {
    const [rows] = await pool.execute(
      `SELECT id, name, formula, notes, created_at AS createdAt, updated_at AS updatedAt
         FROM element_definition
        ORDER BY name ASC`
    )
    res.json({ elements: rows })
  } catch (e) {
    res.status(500).json({ error: 'Failed to list elements', detail: e.message })
  }
})


app.post('/api/plans/:id/elements', async (req, res) => {
  const planId = Number(req.params.id)
  const { elementId } = req.body || {}
  if (!Number.isFinite(planId) || !Number.isFinite(Number(elementId))) {
    return res.status(400).json({ error: 'Invalid plan or element id' })
  }
  try {
    await pool.execute(
      `INSERT IGNORE INTO plan_element (plan_id, element_definition_id)
       VALUES (?, ?)`,
      [planId, Number(elementId)]
    )
    res.status(201).json({ ok: true })
  } catch (e) {
    res.status(500).json({ error: 'Failed to attach element', detail: e.message })
  }
})

// List elements attached to a plan
app.get('/api/plans/:id/elements', async (req, res) => {
  const planId = Number(req.params.id)
  if (!Number.isFinite(planId)) return res.status(400).json({ error: 'Invalid plan id' })
  try {
    const [rows] = await pool.execute(
      `SELECT ed.id, ed.name, ed.formula, ed.notes
         FROM plan_element pe
         JOIN element_definition ed
           ON ed.id = pe.element_definition_id
        WHERE pe.plan_id = ?
    ...
    res.status(500).json({ error: 'Failed to load plan elements', detail: e.message })
  }
})

app.delete('/api/plans/:id/elements/:elementId', async (req, res) => {
  const planId = Number(req.params.id)
  const elementId = Number(req.params.elementId)
  if (!Number.isFinite(planId) || !Number.isFinite(elementId)) {
    return res.status(400).json({ error: 'Invalid ids' })
  }
  try {
    const [r] = await pool.execute(
      `DELETE FROM plan_element
        WHERE plan_id = ? AND element_definition_id = ?`,
      [planId, elementId]
    )
    res.json({ ok: true, removed: r.affectedRows })
  } catch (e) {
    res.status(500).json({ error: 'Failed to detach element', detail: e.message })
  }
})
  ...
  try {
    if (force) {
      ...
      try {
        await conn.beginTransaction()
        await conn.execute(
          'DELETE FROM plan_element WHERE element_definition_id = :id',
          { id }
        )
        await conn.execute(
          'DELETE FROM element_definition WHERE id = :id',
          { id }
        )
        await conn.commit()
        conn.release()
        ...
        conn.release()
        console.error(e)
        return res.status(500).json({ error: 'Force delete failed', detail: e.message })
      }
    }

    // Non-force path: try plain delete first
    await pool.execute('DELETE FROM element_definition WHERE id = :id', { id })
    return res.status(204).end()
  } catch (err) {
    // FK block -> tell client how many usages exist
    if (err && (err.errno === 1451 || err.code === 'ER_ROW_IS_REFERENCED')) {
      const [rows] = await pool.execute(
        'SELECT COUNT(*) AS cnt FROM plan_element WHERE element_definition_id = :id',
        { id }
      )
      const count = rows?.[0]?.cnt ?? 0
      return res.status(409).json({
        error: 'Element is attached to one or more plans',
        count,
        hint: 'Detach from all plans or call with ?force=1 to auto-detach.'
      })
    }
    console.error(err)
    return res.status(500).json({ error: 'Failed to delete element', detail: err.message })
  }
})

app.delete('/api/plans/:planId/elements/:planElementId', async (req, res) => {
  try {
    const planId = Number(req.params.planId)
    const planElementId = Number(req.params.planElementId)
    await pool.execute(
      'DELETE FROM plan_element WHERE id = :planElementId AND plan_id = :planId',
      { planId, planElementId }
    )
    res.status(204).end()
  } catch (err) {
    console.error(err)
    res.status(500).json({ error: 'Failed to detach element from plan', detail: err.message })
  }
})

// GET a single element definition
app.get('/api/element-definitions/:id', async (req, res) => {
  const id = Number(req.params.id)
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'Invalid id' })
  try {
    const [[row]] = await pool.execute(
      `SELECT id, name, scope, formula, notes,
    ...
    res.json({ element: row })
  } catch (e) {
    res.status(500).json({ error: 'Failed to load element', detail: e.message })
  }
})


// UPDATE element definition (supports library formula + params)
app.put('/api/element-definitions/:id', async (req, res) => {
  const id = Number(req.params.id)
  if (!Number.isFinite(id)) return res.status(400).json({ error: 'Invalid id' })
  const { name, formula, notes } = req.body || {}
  if (name !== undefined && !nameRegex.test(name)) {
    return res.status(400).json({ error: 'Invalid element name' })
  }
  try { if (formula !== undefined) validateFormulaSyntax(formula) }
...
app.get('/api/formulas', async (_req, res) => {
  const [rows] = await pool.execute(
    `SELECT id, name, expression, description, variables_json AS variablesJson,
            created_at AS createdAt, updated_at AS updatedAt
     FROM formula_library ORDER BY name ASC`
  )
  res.json(rows)
})
...
app.get('/api/formulas/:id', async (req, res) => {
  ...
  const [[row]] = await pool.execute(
    `SELECT id, name, expression, description, variables_json AS variablesJson,
            created_at AS createdAt, updated_at AS updatedAt
     FROM formula_library WHERE id = :id`, { id }
  )
...
app.post('/api/formulas', async (req, res) => {
  const { name, expression, description = null, variablesJson = null } = req.body
  const [r] = await pool.execute(
    `INSERT INTO formula_library (name, expression, description, variables_json)
     VALUES (:name, :expression, :description, :variablesJson)`,
    { name, expression, description, variablesJson: variablesJson ? JSON.stringify(variablesJson) : null }
  )
...
app.put('/api/formulas/:id', async (req, res) => {
  ...
  await pool.execute(
    `UPDATE formula_library
       SET name = :name, expression = :expression, description = :description,
           variables_json = :variablesJson
     WHERE id = :id`,
    { id, name, expression, description, variablesJson: variablesJson ? JSON.stringify(variablesJson) : null }
  )
  res.status(204).end()
})
...
app.delete('/api/formulas/:id', async (req, res) => {
  const id = Number(req.params.id)
  await pool.execute(`DELETE FROM formula_library WHERE id = :id`, { id })
  res.status(204).end()
})
